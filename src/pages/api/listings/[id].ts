...
import { getFallbackListingById, shouldUseListingFallback } from '../../../lib/listing-fallback';
...
export const GET: APIRoute = async ({ params }) => {
    if (!params.id) return jsonErr(400, 'Listing id required');
    if (shouldUseListingFallback()) {
        const fallback = getFallbackListingById(params.id);
        return fallback ? json(fallback) : jsonErr(404, 'Listing not found');
    }
